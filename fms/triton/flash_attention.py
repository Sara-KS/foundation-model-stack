...
@pytest.mark.parametrize('Z, H, N_CTX_Q, N_CTX_K, D_HEAD',
                         [(4, 48, 1024, 1024, 64),
#                          (4, 48, 8192, 8192, 64), #skip on MI250
#                          (2, 16, 16384, 16384, 128), #skip on MI250
                          (2, 16, 1020, 987, 128),
                          (2, 16, 15498, 2, 128),
                          (2, 16, 7, 16219, 64),
                          (4, 48, 1, 1, 64),
...
def test_op_fwd_bias(Z, H, N_CTX_Q, N_CTX_K, D_HEAD, causal, use_bias, dtype=torch.float16):
    ...
    input_metadata.max_seqlens_q = N_CTX_Q
    input_metadata.max_seqlens_k = N_CTX_K
#skip ones that cause out of memory errors on MI250
    if causal and H==48 and N_CTX_Q > 1000:
        pytest.skip()        
    if causal:
        input_metadata.need_causal()
    if use_bias:
...
def run_benchmark(custom):
    ...
    configs.append(triton.testing.Benchmark(
        ...
        styles=[('red', '-')],
        ylabel='ms',
        plot_name=f'fused-attention-{mode}-causal{causal}-d{head_size}{"-varlen" if varlen else ""}',
        args={
            'D_HEAD': head_size,
            'dtype': dtype,
            'causal': causal,
